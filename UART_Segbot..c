...
void serialRXC(serial_t *s, char data) {
    ...
    if((past[4] == '6')&&(past[9] == '7')){
        //up arrow pressed, speed up '!B516!B507'
        //saturate fwdbackoffset so segbot doesn't fall over
        if(FwdBackOffset <= -2){
            FwdBackOffset = -2;
        }else{
            FwdBackOffset = FwdBackOffset - 0.2;
        }
    }else if((past[4] == '5')&&(past[9] == '6')){
        //down arrow pressed, slow down '!B615!B606'
        //saturate fwdbackoffset so segbot doesn't fall over
        if(FwdBackOffset >= 2){
            FwdBackOffset = 2;
        }else{
    ...
    }else if((past[4] == '4')&&(past[9] == '5')){
        //turn left '!B714!B705'
        if(turnrate > 0){
            //stop turning opposite direction
            turnrate = 0;
        }else{
            turnrate = turnrate - 0.2;
        }
    }else if((past[4] == '3')&&(past[9] == '4')){
        //turn right '!B813!B804'
        if(turnrate < 0){
            //stop turning opposite direction
            turnrate = 0;
        }else{
    ...
    }else if((past[4] == '9')&&(past[9] == ':')){
        //balance still if 2 is pressed
        turnrate = 0;
        FwdBackOffset = 0;
    }
}
